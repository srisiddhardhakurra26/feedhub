...
import { addMastodonUser, deleteSource } from '@/app/actions'

type State = { ok?: true; error?: string } | null

interface MastodonSource {
  id: string
  identifier: string
  label: string | null
  itemCount: number
...
interface Props {
  sources: MastodonSource[]
}

export function MastodonCard({ sources }: Props) {
  const [state, formAction, isPending] = useActionState<State, FormData>(addMastodonUser, null)
    ...
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 space-y-3">
      <div>
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-400">
          Follow a specific Mastodon user. Format: <span className="font-mono">@user@instance.tld</span>
        </div>
      </div>
