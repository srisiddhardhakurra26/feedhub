...
import { addSubstackPub, deleteSource } from '@/app/actions'
...
export function SubstackCard({ sources }: Props) {
  const [state, formAction, isPending] = useActionState<State, FormData>(addSubstackPub, null)
    ...
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 space-y-3">
      <div>
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-400">
          Paste a publication URL (e.g. <span className="font-mono">https://www.platformer.news</span>) or a default slug (e.g. <span className="font-mono">noahpinion</span>).
        </div>
      </div>
      ...
      <form action={formAction} className="flex gap-2 items-center">
        <input
          ...
          type="text"
          required
          placeholder="https://www.example.com  or  slug"
